...
fused_li_manage_mtp(
    ...
    miss_src_ids,                  # int32 [B, 32768]，只写；每请求本轮需要搬入 HBM 的 source token ID 列表，仅前 miss_counts[i] 项有效。
    miss_dst_slots,                # int32 [B, 32768]，只写；与 miss_src_ids 同位置对应的目标 HBM logical slot，仅前 miss_counts[i] 项有效。
...
实现按路数保留性能隔离：Q1–4 继续使用成熟 MrgSort4 路径；Q5–14 全部复用
同一个 SortAll/k-way 泛化函数和状态机，普通 occurrence 使用最多 4096 项的
SortAll，更大集合在该函数内使用 GM-backed k-way fallback。不存在独立 Q8+
算子或另一套宽路处理函数。两类入口对外遵循相同的精确 TopK、union、slot 和
pool mapping 语义。
...
内部排序 payload 使用 15 bit 编码 logical slot：`0..32766` 可表示有效 slot，
`32767` 保留为 packed invalid，并在 `topk_dst_slots` 解码为 `-1`。产品约束进一步
将 cache budget 限制为 `C <= 32640`（保留 slot 32767 作为 invalid 后，
32767 以下最大的 128 倍数）。`INT32_MIN` 只存在于 int32
`cache_slots_pool` 中，不会作为有效 packed slot 写入 payload。
...
source capacity 最大为 21 bit。内部 TopK、LD TopK 和淘汰 candidate 的 payload
统一使用以下编码：

```text
bits 31..17：slot15
bits 16..0 ：source_low17
排序 key bits 3..0：source_high4（仅 source capacity > 2^17 时使用）
```

长度不超过 `2^17 = 131072` 时，payload 的低 17 bit 可直接保存完整 source ID，
不修改排序 key。超过 `2^17` 时，payload 保存 source ID 的低 17 bit，排序 key
低 4 bit 保存 source ID 的高 4 bit；排序和候选选择后再恢复完整 21-bit source
ID。因此长序列范围为 `131072 < SOURCE_CAPACITY <= 2^21`。key tag 最多改变
原始 FP32 排序 key 的 15 ULP；专用 key-tag 回归覆盖 BF16/FP16、全部 4-bit tag
以及 TopK cutoff 附近的边界顺序。
