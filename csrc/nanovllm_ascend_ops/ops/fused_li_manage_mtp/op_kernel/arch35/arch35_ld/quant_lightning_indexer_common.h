...
struct RunInfo {
    ...
    // 官方整行路径的加载循环计数：qScaleLoop 每行 +1（weight/qScale 槽位）、
    // kScaleLoop 每 16 个 s2 块 +1（kScale 槽位）。小 batch 拆 S2 路径不用。
    uint32_t qScaleLoop = 0;
    uint32_t kScaleLoop = 0;

    // MTP classify+publish 需要：cacheRowIdx=reqPoolEntries[bIdx]（cache 槽表基址的 block 数，
    // cacheBase=cacheRowIdx*sourceCapacity）、cacheTokenCount=cacheTokens[bIdx]（当前有效 token
    // 数，决定 top-2048 的 slot 查询范围）。
...
struct ConstInfo {
    ...
    // host workspace 分配的 s1BaseSize（=ceil(256/gSize)）：Branch 2 对齐后计算 s1BaseSize 变
    // 2，但每核 GM segment 步长必须仍用 host 分配值，否则与 host workspace 布局错位。
    uint32_t s1BaseSizeWs = 1ULL;
    ...
    bool isLDOpen = false;
    // Branch 2（整行贪心、无 LD）标志：vector 侧据此走官方 ProcessVec1（weight/qScale
    // 每行仅首块加载、kScale 每 16 块批量加载、16KB kScaleBuf_）。Branch 1 保持逐块加载。
    bool isWholeRowGreedy = false;

    // MTP classify+publish 需要：poolSize=实际 KV pool 的 block 数（IsActiveRequest 校验
    // cacheRowIdx 越界）、cacheSlotsSize=cacheSlotsGm 单行长度（=Align(kSeqSize,128)，
    // slot 查询按 cacheRowIdx*sourceCapacity 定位到该行）。
    uint32_t poolSize = 0;
