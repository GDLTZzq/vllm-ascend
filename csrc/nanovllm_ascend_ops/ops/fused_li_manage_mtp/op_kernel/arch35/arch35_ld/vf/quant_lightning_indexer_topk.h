...
public:
    ...
    {
        // 2 * QLILdCommon::Align(topK, (uint32_t)256):两块hisIndexLocal；3 * 256：histogramsLocal idxHighLocal idxLowLocal；64：nkValueLocal
        uint64_t bufferSize1 = (2 * QLILdCommon::Align(topK, (uint32_t)256) + 3 * 256 + 64)  * sizeof(uint32_t);
        // QLILdCommon::Align(topK, (uint32_t)256) + trunkLen：tmpIndexLocal
        uint64_t bufferSize2 = (QLILdCommon::Align(topK, (uint32_t)256) + trunkLen)  * sizeof(uint16_t);
    ...
    __aicore__ inline void InitBuffers(LocalTensor<uint32_t>& sharedTmpBuffer)
    {
        ...
        LocalTensor<uint32_t> hisIndexLocal2 = hisIndexLocal1[QLILdCommon::Align(topK, (uint32_t)256)];
        hisIndexLocal[0] = hisIndexLocal1;
        hisIndexLocal[1] = hisIndexLocal2;
        histogramsLocal = hisIndexLocal2[QLILdCommon::Align(topK, (uint32_t)256)];
    ...
    __aicore__ inline void TopK(LocalTensor<uint16_t>& mrgValueLocal, LocalTensor<uint32_t>& indicesOutLocal,
                                    LocalTensor<uint16_t>& hisValueLocal, uint32_t s2SeqLen, uint32_t loopIdx, uint32_t s2LoopNum, bool isNeedLD)
    {
        // true: 开启返回hisValueLocal；恒 true 以便取 kth value（阈值）
        if (s2LoopNum == 1) {
            LdTopkB16Gather::LiTopKVF<true>(tmpIndexLocal, hisValueLocal, mrgValueLocal, histogramsLocal, idxHighLocal, idxLowLocal, nkValueLocal, topK, s2SeqLen);
            PipeBarrier<PIPE_V>();
        ...
        if (loopIdx == 0 && !isNeedLD) {
            ...
            LdTopkB16Gather::LiTopKVF<true>(tmpIndexLocal, hisValueLocal, mrgValueLocal, histogramsLocal, idxHighLocal, idxLowLocal, nkValueLocal, topK, s2SeqLen);
            PipeBarrier<PIPE_V>();
            LdTopkB16Gather::LiTopKGatherVF(hisIndexLocal[(loopIdx + 1) % 2], hisValueLocal, mrgValueLocal, tmpIndexLocal,
                                          hisIndexLocal[loopIdx % 2], topK, loopIdx * trunkLen - QLILdCommon::Align(topK, (uint32_t)256), s2SeqLen);
            if (loopIdx == s2LoopNum - 1) {
                PipeBarrier<PIPE_V>();
                AscendC::DataCopy(indicesOutLocal, hisIndexLocal[(loopIdx + 1) % 2], QLILdCommon::Align(topK, (uint32_t)256));
            }
        }
        ...
        if (loopIdx == 0 && isNeedLD) {
            ...
            LdTopkB16Gather::LiTopKVF<true>(tmpIndexLocal, hisValueLocal, mrgValueLocal, histogramsLocal, idxHighLocal, idxLowLocal, nkValueLocal, topK, s2SeqLen);
            PipeBarrier<PIPE_V>();
            LdTopkB16Gather::LiTopKGatherVF(hisIndexLocal[(loopIdx + 1) % 2], hisValueLocal, mrgValueLocal, tmpIndexLocal, hisIndexLocal[loopIdx % 2],
                                          topK, loopIdx * trunkLen - QLILdCommon::Align(topK, (uint32_t)256), s2SeqLen);
            PipeBarrier<PIPE_V>();
            AscendC::DataCopy(indicesOutLocal, hisIndexLocal[(loopIdx + 1) % 2], QLILdCommon::Align(topK, (uint32_t)256));
        }
    }
    __aicore__ inline void LdTopK(LocalTensor<uint16_t>& mrgValueLocal, LocalTensor<uint32_t> indexLocal, LocalTensor<uint32_t>& indicesOutLocal,
                                LocalTensor<uint16_t>& hisValueLocal, uint32_t s2SeqLen, uint32_t loopIdx, uint32_t s2LoopNum)
    {
        LdTopkB16Gather::LiTopKVF<true>(tmpIndexLocal, hisValueLocal, mrgValueLocal, histogramsLocal, idxHighLocal, idxLowLocal, nkValueLocal, topK, s2SeqLen);
        PipeBarrier<PIPE_V>();
        LdTopkB16Gather::LiTopKLDGatherVF(indicesOutLocal, tmpIndexLocal, indexLocal, topK);
    }

    __aicore__ inline uint16_t GetLastKthValue()
    {
        return static_cast<uint16_t>(nkValueLocal.GetValue(0) & 0xffffu);
    }
