 ...
 * @param topK topK元素
 * @param validLen 有效元素个数:QLILdCommon::Align(topkCountAlign256_ + validTrunkLen, (uint32_t)256)
 */
                                ...
                                uint32_t validLen)
{
    ...
    // filter
    AscendC::Duplicate(tmpIdxLocal, (uint16_t)(0), QLILdCommon::Align(topK, (uint32_t)128));
    // 输出大于k-value的值idx
...
// Fill slot bits for survivors whose absolute source index belongs to the
// slot chunk currently resident in UB. 32-bit in-place variant: the MTP
// publish path feeds indicesOutLocal_ for both src and dst. On the first
// chunk it still holds raw absolute token ids; from the second chunk on it
// holds payload (slot<<18|source). Masking currentIdx to the low 18 bits
// (the token id) keeps the range test and gather index correct in both cases,
// and clamping localIdx to [0, chunkLen) bounds every gather inside the
// resident chunk so out-of-chunk lanes are discarded, never OOB-read.
__simd_vf__ void ApplyStreamingSlotChunkVFImpl32(
    __ubuf__ uint32_t *outputPayloadBuf,
    __ubuf__ uint32_t *srcBuf,
    ...
    uint16_t vfLoop)
{
    ...
    MicroAPI::RegTensor<uint32_t> chunkCapVec;
    ...
    MicroAPI::Duplicate(chunkCapVec, chunkLen - 1U);
    ...
    for (uint16_t i = 0; i < vfLoop; ++i) {
        ...
        MicroAPI::LoadAlign<uint32_t, MicroAPI::LoadDist::DIST_NORM>(
            currentIdx, srcBuf + i * 64);
        MicroAPI::And(currentIdx, currentIdx, tokenMaskVec, allB32);
        ...
        MicroAPI::Select(localIdx, localIdx, zeroVec, geBase);
        MicroAPI::Select(localIdx, localIdx, chunkCapVec, ltEnd);
