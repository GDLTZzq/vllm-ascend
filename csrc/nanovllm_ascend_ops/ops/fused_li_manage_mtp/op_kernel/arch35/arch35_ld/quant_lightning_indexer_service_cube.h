...
public:
    ...
    __aicore__ inline void ComputeMm1(const QLILdCommon::RunInfo &runInfo);
...
protected:
    __aicore__ inline void Fixp(uint64_t s1gGmOffset, uint64_t s2GmOffset, uint64_t s1gL0RealSize,
                                uint64_t s2L0RealSize, const QLILdCommon::RunInfo &runInfo);
    __aicore__ inline void ComuteL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize, const QLILdCommon::RunInfo &runInfo);
    __aicore__ inline void LoadKeyToL0b(uint64_t s2L0Offset, uint64_t s2L1RealSize, uint64_t s2L0RealSize,
                                        const QLILdCommon::RunInfo &runInfo);
    __aicore__ inline void LoadQueryToL0a(uint64_t s1gL1Offset, uint64_t s1gL0Offset, uint64_t s1gL1RealSize,
                                          uint64_t s1gL0RealSize, const QLILdCommon::RunInfo &runInfo);
    __aicore__ inline void QueryNd2Nz(uint64_t s1gL1RealSize, uint64_t s1gL1Offset, const QLILdCommon::RunInfo &runInfo);
    __aicore__ inline void KeyNd2Nz(uint64_t s2L1RealSize, uint64_t s2GmOffset, const QLILdCommon::RunInfo &runInfo);
    __aicore__ inline void KeyNd2NzForPA(uint64_t s2L1RealSize, uint64_t s2GmOffset, const QLILdCommon::RunInfo &runInfo);
...
template <typename QLIT>
__aicore__ inline void QLIMatmul<QLIT>::ComputeMm1(const QLILdCommon::RunInfo &runInfo)
{
    CrossCoreWaitFlag<QLILdCommon::ConstInfo::QLI_SYNC_MODE4, PIPE_FIX>(QLILdCommon::ConstInfo::CROSS_VC_EVENT + runInfo.loop % 2);
    CrossCoreWaitFlag<QLILdCommon::ConstInfo::QLI_SYNC_MODE4, PIPE_FIX>(QLILdCommon::ConstInfo::CROSS_VC_EVENT + runInfo.loop % 2 + QLILdCommon::ConstInfo::AIV0_AIV1_OFFSET);
    ...
    if (GetBlockIdx() == 12U && runInfo.loop == 0U) {
        AscendC::printf("[C8BISECT] AIC12 ComputeMm1 enter s1gProc=%u s2Proc=%u s2Idx=%u\n",
                        (uint32_t)s1gProcessSize, (uint32_t)s2ProcessSize, (uint32_t)runInfo.s2Idx);
    }
#endif
    ...
    CrossCoreSetFlag<QLILdCommon::ConstInfo::QLI_SYNC_MODE4, PIPE_FIX>(QLILdCommon::ConstInfo::CROSS_CV_EVENT + runInfo.loop % 2);
    CrossCoreSetFlag<QLILdCommon::ConstInfo::QLI_SYNC_MODE4, PIPE_FIX>(QLILdCommon::ConstInfo::CROSS_CV_EVENT + runInfo.loop % 2 + QLILdCommon::ConstInfo::AIV0_AIV1_OFFSET);
}
                                                    ...
                                                    const QLILdCommon::RunInfo &runInfo)
{
    ...
    while (s2L1Offset < s2L1RealSize) {
        ...
        uint64_t keyGmOffset = blkTableGm_.GetValue(runInfo.bIdx * constInfo_.maxBlockNumPerBatch + s2BlkId) * constInfo_.keyStride0 + 
...
#if C8_MTP_BISECT_PRINT
        if (GetBlockIdx() == 12U && runInfo.loop == 0U) {
            AscendC::printf("[C8BISECT] AIC12 Fixp enter mSize=%u nSize=%u ndNum=%u dstNd=%u dstStride=%u\n",
                            mSize, fixpipeParams.nSize, fixpipeParams.params.ndNum,
                            fixpipeParams.params.dstNdStride, fixpipeParams.dstStride);
        }
#endif
#if C8_MTP_BISECT_SKIP_FIXPIPE
        // BISECT：跳过 Fixpipe（L0C->UB）。M=128 时 dualDstCtl=1 的写入范围疑似
        // 超出 128KB mm1ResUB_。跳过则 AIV 读未初始化 UB（不越界），若不再崩 → Fixpipe 越界。
        (void)0;
#else
...
#if C8_MTP_BISECT_PRINT
        if (GetBlockIdx() == 12U && runInfo.loop == 0U) {
            AscendC::printf("[C8BISECT] AIC12 Fixp done mSize=%u\n", mSize);
        }
#endif
    } else {
