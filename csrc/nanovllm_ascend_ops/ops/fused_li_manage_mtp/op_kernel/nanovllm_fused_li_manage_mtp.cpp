...
// arch35 (A5/Ascend950) vs arch22 (A3/910B & 910_93): only one arch's
// headers enter a TU, so the gate must be identical to every arch switch in
// this file.  The arch35 side drives the self-contained C8 fp8 engine through
// the adapter header; arch22 keeps its own real headers.
// A 0/1 numeric macro is required: a `defined` token inside an object-like
// macro that is then expanded under `#if` is undefined behavior and trips the
// device compiler's -Werror,-Wexpansion-to-defined.
#if (__CCE_AICORE__ == 310) || (__CCE_AICORE__ == 200)
#define NANO_VLLM_FUSED_MTP_ARCH35 1
#elif defined(__DAV_310R6__)
#define NANO_VLLM_FUSED_MTP_ARCH35 1
#else
#define NANO_VLLM_FUSED_MTP_ARCH35 0
#endif
#if NANO_VLLM_FUSED_MTP_ARCH35
#include "arch35/fused_li_manage_mtp_arch35.h"
#else
#include "arch22/lightning_indexer_kernel.h"
#include "arch22/fused_li_manage_mtp_union.h"
#endif
...
#endif  // !NANO_VLLM_FUSED_MTP_ARCH35 (arch22 helper functions and macros)
...
{
#if NANO_VLLM_FUSED_MTP_ARCH35
    // arch35 (Ascend950): dispatch to the C8 fp8 LD engine under the shared A3
    // ABI.  DT is intentionally left unused on this side (only the fp8 tiling
    // key reaches the 950 host path), so the templated instantiation mechanism
    // is unchanged for both arch builds.
    //
    // This kernel is templated on DT (three instantiations via the shared
    // template_tiling_key.h), so the tiling is read with the template-safe
    // GET_TILING_DATA_WITH_STRUCT pattern (see the arch22 path below and the
    // validated a5_fused_li_manage_c8 kernel) instead of the non-templated
    // REGISTER_TILING_DEFAULT + GET_TILING_DATA pair.  AscendC flat tiling
    // structs are raw-copy compatible, so the plain A5FusedLiManageMtpC8TilingData
    // image written by the fp8 host path (GetTilingData<T>() over the raw tiling
    // buffer) is read back directly.
    TPipe tPipe;
    __gm__ uint8_t *user = GetUserWorkspace(workspace);
    KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
    GET_TILING_DATA_WITH_STRUCT(A5FusedLiManageMtpC8TilingData, tilingData, tiling);
    arch35_run_fused_li_manage_mtp(
        &tPipe, &tilingData, user,
        weights, queryDequantScale, query,
        keyDequantScale, key, blockTable,
        actualSeqLengthsQuery, actualSeqLengthsKey,
        offloadSeqLengthsKey, cacheTokens,
        requestState, reqPoolEntries, cacheSlots,
        topkSourceIds, topkSlots, topkMissCounts,
        missSrcIds, missDstSlots, missCount,
        cacheSlotsOut);
#else
    TPipe tPipe;
