...
// Ascend950 (arch35) fp8 path tiling data, registered under the fp8 tiling key
// (LI_MTP_TPL_FP8 == 55).  Field-for-field byte-compatible with the plain
// A5FusedLiManageMtpC8TilingData read by the arch35 C8 engine
// (op_kernel/arch35/a5_fused_li_manage_mtp_c8_tiling.h) -- both are contiguous
// uint32 images.
//
// The op registers FusedLiManageMtpTilingData as its default class, and
// TilingContext::GetTilingData<T>() only resolves a type the op has registered
// (sizing the raw tiling buffer from that class as well).  Without this
// registration the fp8 host path gets a null pointer from
// GetTilingData<A5FusedLiManageMtpC8TilingData>() and, worse, only 40 bytes of
// tiling buffer for a 56-byte struct.
BEGIN_TILING_DATA_DEF(A5LiManageMtpC8Tiling)
TILING_DATA_FIELD_DEF(uint32_t, usedCoreNum)
TILING_DATA_FIELD_DEF(uint32_t, batchSize)
TILING_DATA_FIELD_DEF(uint32_t, packedQueryCount)
TILING_DATA_FIELD_DEF(uint32_t, poolSize)
TILING_DATA_FIELD_DEF(uint32_t, sourceCapacity)
TILING_DATA_FIELD_DEF(uint32_t, indexHeads)
TILING_DATA_FIELD_DEF(uint32_t, maxBlockNumPerBatch)
TILING_DATA_FIELD_DEF(uint32_t, maxCandidateLen)
TILING_DATA_FIELD_DEF(uint32_t, keyStride)
TILING_DATA_FIELD_DEF(uint32_t, scaleStride)
TILING_DATA_FIELD_DEF(uint32_t, scoreWorkspaceStride)
TILING_DATA_FIELD_DEF(uint32_t, queryTileSize)
TILING_DATA_FIELD_DEF(uint32_t, splitEnable)
TILING_DATA_FIELD_DEF(uint32_t, scoringCoreNum)
END_TILING_DATA_DEF
REGISTER_TILING_DATA_CLASS(NanovllmFusedLiManageMtp_55, A5LiManageMtpC8Tiling)
...
public:
    ...
    // Ascend950 (arch35) fp8 dedicated path. Reads the A3-ordered inputs, fills
    // A5FusedLiManageMtpC8TilingData + workspace and selects the fp8 tiling key.
    // Must run before ParseAndCheck because CheckDtype rejects fp8.
    ge::graphStatus TilingC8Fp8(FusedLiManageMtpTilingInfo &tilingInfo);
...
#endif // NANOVLLM_FUSED_LI_MANAGE_MTP_TILING_H_
