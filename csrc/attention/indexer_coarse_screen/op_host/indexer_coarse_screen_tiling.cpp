...
{
    opParamInfo_.attenOut.desc = context_->GetOutputDesc(INDEXER_COARSE_SCREEN);
    opParamInfo_.attenOut.shape = context_->GetOutputShape(INDEXER_COARSE_SCREEN);
}
...
ge::graphStatus IndexerCoarseScreenInfoParser::GetQueryKeyAndOutLayout()
{
...
ge::graphStatus IndexerCoarseScreenInfoParser::GetS1Size()
{
    if (qLayout_ == DataLayout::BSND) {
...
ge::graphStatus IndexerCoarseScreenTiling::DoTiling(IndexerCoarseScreenTilingInfo *tilingInfo)
{
    ...
    //   wBarGm [bSize*Align16(gSize)] bf16(行步长对齐 16 元素=32B,规避 GM 写对齐限制)
    workspaceSize += static_cast<uint64_t>(tilingInfo->bSize) * tilingInfo->gSize * HEAD_DIM_LIMIT *
                     sizeof(uint16_t); // qBarGm
    uint32_t wBarStride = (tilingInfo->gSize + 15U) / 16U * 16U;
    workspaceSize += static_cast<uint64_t>(tilingInfo->bSize) * wBarStride * sizeof(uint16_t); // wBarGm
...
ge::graphStatus TilingForIndexerCoarseScreen(gert::TilingContext *context)
{
    ...
    IndexerCoarseScreenInfoParser IndexerCoarseScreenInfoParser(context);
    if (IndexerCoarseScreenInfoParser.ParseAndCheck(liInfo) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
