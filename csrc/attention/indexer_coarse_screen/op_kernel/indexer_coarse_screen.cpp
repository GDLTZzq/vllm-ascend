...
#include "indexer_coarse_screen_template_tiling_key.h"
    ...
    #include "arch35/indexer_coarse_screen_kernel.h"
...
#define INVOKE_LI_NO_KFC_OP_IMPL(templateClass, ...)                                                                   \
    do {                                                                                                               \
        ...
        op.Init(query, key, weights, rowWeights, actualSeqLengthsQ, actualSeqLengths, blocktable, sparseIndices, user,  \
                tiling_data, &tPipe);                                                                                  \
        op.Process();                                                                                                  \
    } while (0)
                                                 ...
                                                 __gm__ uint8_t *tiling)
{
    ...
    if (ORIG_DTYPE_QUERY == DT_BF16) {
        INVOKE_LI_NO_KFC_OP_IMPL(IndexerCoarseScreenKernel, bfloat16_t, bfloat16_t, int32_t,
            PAGE_ATTENTION, LI_LAYOUT(LAYOUT_T), LI_LAYOUT(K_LAYOUT_T), DT_W_FLAG);
    } else if (ORIG_DTYPE_QUERY == DT_FLOAT16) {
        INVOKE_LI_NO_KFC_OP_IMPL(IndexerCoarseScreenKernel, half, half, int32_t,
            PAGE_ATTENTION, LI_LAYOUT(LAYOUT_T), LI_LAYOUT(K_LAYOUT_T), DT_W_FLAG);
    }
#else
    if constexpr (DT_Q == LI_TPL_FP16 && DT_K == LI_TPL_FP16 && DT_OUT == LI_TPL_INT32) {
        INVOKE_LI_NO_KFC_OP_IMPL(IndexerCoarseScreenKernel, half, half, int32_t, PAGE_ATTENTION,
                                 LI_LAYOUT(LAYOUT_T), LI_LAYOUT(K_LAYOUT_T), DT_W_FLAG);
    } else {
        INVOKE_LI_NO_KFC_OP_IMPL(IndexerCoarseScreenKernel, bfloat16_t, bfloat16_t, int32_t, PAGE_ATTENTION,
                                 LI_LAYOUT(LAYOUT_T), LI_LAYOUT(K_LAYOUT_T), DT_W_FLAG);
    }
#endif
}
